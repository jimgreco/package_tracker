...
@main struct PorchPongApp: App {
  ...
  init() {
    ...
    try? FileManager.default.removeItem(
      at: FileManager.default.temporaryDirectory.appendingPathComponent("PorchPongExports"))
  }
  var body: some Scene {
    WindowGroup {
      RootView().environment(store).tint(.porchPong)
    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 28) {
        ...
        VStack(alignment: .leading, spacing: 12) {
          Text("PorchPong").font(.system(.largeTitle, design: .rounded, weight: .bold))
        ...
        Link("Open PorchPong website", destination: Configuration.origin).frame(minHeight: 44)
  ...
  var body: some View {
    Group {
      if let data, let image = UIImage(data: data) {
        ...
        Image(systemName: "shippingbox").font(.title2).foregroundStyle(Color.porchPong).frame(
          maxWidth: .infinity, maxHeight: .infinity
        ).background(Color.porchPong.opacity(0.08))
      }
...
struct StatusLabel: View {
  ...
  var body: some View {
    ...
    ).foregroundStyle(status == "delivered" ? Color.secondary : Color.porchPong).padding(
      .vertical, 5)
  }
}
