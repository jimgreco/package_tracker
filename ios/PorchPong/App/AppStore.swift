...
@MainActor @Observable final class AppStore {
  ...
  init(
    api: any PorchPongAPI = APIClient(), session: NativeSession? = nil,
  ...
  func signOut() async {
    ...
    do { try await api.revoke(old) } catch {
      if session == nil {
        errorMessage =
          "Signed out on this iPhone. PorchPong could not revoke the server session; it will expire automatically."
      }
    }
  }
  ...
  func friendly(_ error: Error) -> String {
    if error is DecodingError {
      return
        "PorchPong returned data this version could not read. Pull to refresh, or try again later."
    }
    if error is URLError { return "Could not reach PorchPong. Check your connection and try again." }
  ...
  func export() async throws -> URL {
    guard canWrite, let household = householdId else {
      throw APIError(status: 0, message: "Connect to PorchPong to export your household.")
    }
    let captured = epoch
    let data = try await api.request(
      "settings/export", method: "GET", body: nil, key: nil, household: household)
    guard captured == epoch else { throw CancellationError() }
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("PorchPongExports")
    try FileManager.default.createDirectory(
      at: directory, withIntermediateDirectories: true,
      attributes: [.protectionKey: FileProtectionType.complete])
    let url = directory.appendingPathComponent("porchpong-\(UUID()).json")
    ...
    if let body, body != payload {
      if unresolved {
        throw APIError(
          status: 0,
          message:
            "The previous save may have reached PorchPong. Restore the previous details and retry that save before starting another package."
        )
      }
      key = UUID().uuidString
    }
