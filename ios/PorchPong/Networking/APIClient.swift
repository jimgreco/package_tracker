...
protocol PorchPongAPI: Sendable {
...
actor APIClient: PorchPongAPI {
  ...
  func request(
    ...
    else { throw APIError(status: 400, message: "Invalid PorchPong address.") }
    ...
    guard let http = response as? HTTPURLResponse else {
      throw APIError(status: 502, message: "PorchPong returned an unreadable response.")
    }
    guard (200..<300).contains(http.statusCode) else {
      struct Failure: Decodable { var error: String }
      let message =
        (try? JSONDecoder().decode(Failure.self, from: data).error)
        ?? "PorchPong is unavailable. Try again shortly."
      throw APIError(
