  ...
  var body: some View {
    List {
      ...
      if let settings = store.dashboard?.settings {
        ...
        Section("Forwarding") {
          Text(
            "Forward order and shipping emails to your household’s private address. PorchPong checks for physical deliveries."
          ).font(.footnote)
        ...
        Section("Google Calendar") {
          ...
          Text(
            "PorchPong creates a Package Deliveries calendar for this household. It can only manage calendars it creates, not your unrelated calendars."
          ).font(.footnote)
          Button(
            calendar.busy
              ? "Connecting…"
          ...
          Text(
            "To see deliveries in Apple Calendar, add your Google account in iPhone Settings → Apps → Calendar → Calendar Accounts. Enable Calendars, then select the PorchPong calendar in the Calendar app."
          ).font(.footnote)
          Text("Imports and calendar updates continue while PorchPong is closed.").font(.caption)
            .foregroundStyle(.secondary)
        }
