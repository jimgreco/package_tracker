...
struct WalkthroughView: View {
  ...
  private var titles: [String] {
    [
      "Welcome to PorchPong", "A heads-up when it matters", "Deliveries, on your calendar",
    ...
    NavigationStack {
      ScrollView {
        VStack(alignment: .leading, spacing: 24) {
          ...
          .font(.system(size: 52)).foregroundStyle(Color.porchPong).accessibilityHidden(true)
          ...
          if step == 0 {
            ...
            if PushNotifications.shared.authorization == .denied {
              Text(
                "Notifications are blocked. Open iPhone settings and turn on Allow Notifications for PorchPong."
              )
              Link(
            ...
            Text(
              "PorchPong adds a Package Deliveries calendar to Google Calendar and keeps delivery estimates up to date for this household."
            )
            Text(
              "Access is limited to calendars PorchPong creates. Your other calendars stay private."
  ...
  private func feature(_ title: String, _ text: String, _ icon: String) -> some View {
    HStack(alignment: .top, spacing: 16) {
      Image(systemName: icon).font(.title2).foregroundStyle(Color.porchPong).frame(width: 28)
