...
@MainActor @Observable final class PushNotifications {
  ...
  func enable() async throws {
    guard
      ...
      throw APIError(status: 0, message: "Allow notifications for Doorstep in iPhone Settings.")
    }
    await refreshAuthorization()
...
struct NotificationSettingsSection: View {
  ...
  var body: some View {
    Section("Notifications") {
      Text(
        "Your preferences for this household. Alerts show a general message; package details stay in Doorstep."
      ).font(.footnote)
      if let prefs {
        if !prefs.configured {
