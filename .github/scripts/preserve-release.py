...
os.umask(0o077)
dest = Path.home() / 'Library/Logs/DoorstepReleases' / (os.environ['GITHUB_RUN_ID'] + '-' + os.environ['GITHUB_RUN_ATTEMPT']) / os.environ['GITHUB_JOB']
...
ipa = temp / 'export/Doorstep.ipa'
if ipa.is_file():
    shutil.copy2(ipa, dest / ipa.name)
archive = temp / 'Doorstep.xcarchive'
if archive.is_dir():
